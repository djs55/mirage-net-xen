...
module Make
  ...
  type transport = {
    ...
    rx_ring: (RC.response,int) Ring.Front.t;
    ...
    then return ()
      ...
      List.iter
        (fun (gref', page) ->
          ...
          t.transport.rx_next_id <- (t.transport.rx_next_id + 1) mod (Array.length nf.rx_shares  );
  ...
  let rx_poll t =
    Ring.Front.ack_responses t.transport.rx_ring (fun slot ->
      ...
      |sz when status > 0 ->
        ...
        Stats.rx sz;
        ignore_result
        (try_lwt t.transport.receive_callback packet
  ...
  let poll_thread (t: t) : unit Lwt.t =
    let rec loop from =
      ...
      >>= fun () ->
      rx_poll t;
      Ring.Front.poll nf.transport.tx_ring TX.Proto_64.read;
  ...
  let make b =
    ...
    let t = { id; backend_id; tx_ring; tx_mutex; tx_next_id;
      rx_ring; rx_shares; rx_pages; rx_next_id; stats;
      channel; mac; backend; features; frontend
    } in
  ...
  let write_request ?size ~flags t page =
    ...
    if page.Cstruct.off + len > page_size then begin
      ...
      let gref = M.int32_of_grant gref in
      Ring.Front.write t.transport.tx_ring (TX.Proto_64.write ~id ~gref ~offset ~flags ~size)
      >>= fun replied_t ->
